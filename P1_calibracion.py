...
for i in range(2):
    ...
    for j in range(2):
        ...
        ajuste = np.polyfit(data_out[i,int(fs*0.1):-int(fs*0.1),j],data_in[i,int(fs*0.1):-int(fs*0.1),j],1)
        
        
        fig = plt.figure(figsize=(14, 7), dpi=250)
        ax = fig.add_axes([.12, .15, .75, .8])        
        ax.plot(data_out[i,int(fs*0.1):-int(fs*0.1),j],data_in[i,int(fs*0.1):-int(fs*0.1),j],'--',color='blue',alpha=0.8,label='Señal')
        ax.plot(data_out[i,int(fs*0.1):-int(fs*0.1),j],data_out[i,int(fs*0.1):-int(fs*0.1),j]*ajuste[0]+ajuste[1],'--',color='red',alpha=0.8,label='Ajuste')
        ax.grid(linestyle='--')
        ax.legend(loc=4)
        ...
        ax.set_title(u'Señal enviada y adquirida en ' + canales[j] + ' utilizando función ' + formas[i] )
        ...
        fig = plt.figure(figsize=(14, 7), dpi=250)
        ax = fig.add_axes([.12, .15, .75, .8])        
        ax.plot(data_out[i,int(fs*0.1):-int(fs*0.1),j],(data_in[i,int(fs*0.1):-int(fs*0.1),j]-ajuste[1])/ajuste[0],'--',color='red')       
        ax.set_xlabel('Señal enviada [V]')
